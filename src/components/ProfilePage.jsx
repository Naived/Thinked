...
const ProfileInput = ({ label, id, type, value, onChange }) => (
    ...
    const handleSaveChanges = async (e) => {
        ...
        try {
            await updateDoc(userDocRef, {
                ...
                // Note: Email cannot be changed directly here for security reasons.
            });
                    ...
                    <div className="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl rounded-2xl bg-clip-border">
                        <div className="border-black/12.5 rounded-t-2xl border-b-0 border-solid p-6 pb-0">
                            <div className="flex items-center">
                                ...
                                <button type="button" onClick={() => setIsEditing(!isEditing)} className="inline-block px-8 py-2 mb-4 ml-auto font-bold leading-normal text-center text-white align-middle transition-all ease-in bg-blue-500 border-0 rounded-lg shadow-md cursor-pointer text-xs tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">
                                    {isEditing ? 'Cancel' : 'Edit'}
                                </button>
                                {isEditing && (
                                    <button type="submit" disabled={isLoading} className="inline-block px-8 py-2 mb-4 ml-2 font-bold leading-normal text-center text-white align-middle transition-all ease-in bg-emerald-500 border-0 rounded-lg shadow-md cursor-pointer text-xs tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">
                        ...
                        <div className="flex-auto p-6">
                            ...
                            <div className="flex flex-wrap -mx-3">
                                <ProfileInput label="Username" id="username" type="text" value={userData.username || ''} onChange={handleInputChange} />
                                <ProfileInput label="Email Address" id="email" type="email" value={userData.email || ''} onChange={handleInputChange} />
                                <ProfileInput label="Full Name" id="fullName" type="text" value={userData.fullName || ''} onChange={handleInputChange} />
                            </div>
                        </div>
                    </div>
                </form>
